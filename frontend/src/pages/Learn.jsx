...
import {
    ...
    Award, BookOpen, FileText, Play, X
} from "lucide-react";
...
// check if user is allowed to access this course
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col">
      ...
      {/* ── TOP BAR ── */}
      <div className="bg-gray-900 border-b border-gray-800 px-3 md:px-4 py-3 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <h1 className="text-white font-semibold text-xs md:text-sm line-clamp-1">
              {currentCourse.title}
            </h1>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {progress?.isCompleted && (
            <button className="hidden sm:flex items-center gap-1.5 bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 text-xs font-semibold px-3 py-1.5 rounded-lg">
          ...
          >
            ...
            <span className="hidden sm:inline">{sidebarOpen ? "Hide" : "Show"}</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className={`flex flex-col overflow-y-auto transition-all duration-300
          ${sidebarOpen ? "w-full md:flex-1" : "flex-1"}`}
        >
          {/* video player */}
          <div className="bg-black w-full flex items-center justify-center"
            style={{ height: "420px", minHeight: "200px" }}
          >
            {currentLesson?.videoUrl ? (
              ...
              <div className="text-center px-4">
                ...
                <p className="text-gray-600 text-xs">
                  {currentLesson ? "No video" : "Select a lesson"}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="p-4 border-b border-gray-800">
            <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
              <div className="min-w-0 flex-1">
                <h2 className="text-white font-bold text-base md:text-xl mb-1 line-clamp-2">
                  {currentLesson?.title}
                </h2>
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all shrink-0
                    ${isLessonCompleted(currentLesson?._id)
                      ...
                      : "bg-gray-800 border border-gray-700 text-gray-300 hover:border-emerald-500"
                    }`}
                >
                  {isLessonCompleted(currentLesson?._id)
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                disabled={isFirst}
                className="flex items-center gap-1.5 px-3 py-2 bg-gray-800 border border-gray-700 text-gray-300 rounded-xl text-xs hover:border-gray-600 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                disabled={isLast}
                className="flex items-center gap-1.5 px-3 py-2 bg-emerald-500 text-white rounded-xl text-xs font-semibold hover:bg-emerald-600 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Next
              ...
              {isLast && quiz && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 bg-purple-500/15 border border-purple-500/30 text-purple-400 rounded-xl text-xs font-semibold ml-auto"
                >
              ...
              <div className="flex flex-col gap-2">
                {currentLesson.resources.map((url, i) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-2 text-emerald-500 text-xs hover:text-emerald-400 transition-colors"
                  >
                    <FileText className="w-3.5 h-3.5" />
                    Resource {i + 1}
                  </a>
        ...
        {sidebarOpen && (
          <div className={`
            bg-gray-900 border-gray-800 flex flex-col shrink-0 overflow-y-auto
            
            /* mobile — full width overlay */
            absolute inset-0 z-10 border-0
            
            /* desktop — right panel */
            md:relative md:inset-auto md:w-72 md:border-l
          `}>
            ...
            <div className="flex flex-col flex-1">
              {currentCourse.lessons?.map((lesson, i) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 px-3 py-3 text-left border-b border-gray-800/50 transition-all
                      ${isActive
              ...
              {quiz && (
                <button
                  ...
                  className="flex items-center gap-3 px-3 py-3 text-left hover:bg-gray-800/50 transition-all border-b border-gray-800/50"
                >
                  <FileText className="w-4 h-4 text-purple-400 shrink-0" />
                  <div>
                    <p className="text-xs font-medium text-purple-400">Final Quiz</p>
                    <p className="text-gray-600 text-xs">
